...
func _on_ez_dialogue_custom_signal_received(value: String):
	...
	if params[0] == "emote":
		var seleciona_emote = params [1]
		if seleciona_emote == "monstro":
			animação_monstro.emit()
		if seleciona_emote == "idle":
			animação_idle.emit()
